...
//--- Create test Managed Instance inside the instance pool setting up TLS and connection type (proxy redirect in PowerShell)
...
resource "null_resource" "create_managed_instance" { 
    ...
    provisioner local-exec {
        ...
        command = <<EOT
            ...
            New-AzSqlInstance `
                ...
                -VCore ${var.mi_vcore_capacity} `
                -Edition ${var.mi_edition} `
                -ComputeGeneration ${var.mi_compute_generation} `
                -LicenseType ${var.mi_license_type} `
    ...
    // Managed Instance must be created after Instance Pool has been deployed
    depends_on = [
        null_resource.create_instance_pool
    ]
}
