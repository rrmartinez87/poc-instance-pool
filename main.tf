...
resource "null_resource" "create_instance_pool" { 
  provisioner local-exec {
      command = "az sql instance-pool create --name ${join(local.separator, [var.instance_pool_name, random_uuid.poc.result])} --capacity ${var.capacity} --edition ${var.edition} --family ${var.family} --license-type ${var.license_type} --location ${azurerm_resource_group.rg.location} --resource-group ${azurerm_resource_group.rg.name} --subnet ${azurerm_subnet.subnet.id}"
  }
}
*/
/*
// Create test Managed Instance in pool
resource "null_resource" "create_managed_database" { 
  provisioner local-exec {
    command = "az sql midb create --managed-instance ${var.managed_instance_name} --name ${var.managed_database_name} --resource-group ${azurerm_resource_group.rg.name}"
  }
}

// Create test Managed Database in managed instance
resource "null_resource" "create_managed_database" { 
  provisioner local-exec {
    command = "az sql midb create --managed-instance ${var.managed_instance_name} --name ${var.managed_database_name} --resource-group ${azurerm_resource_group.rg.name}"
  }
