pipeline {
    ...
    stages {
        ...
        steps {
            git branch: 'master', credentialsId: 'Github', url: 'https://github.com/rrmartinez87/poc-instance-pool.git'
            }
        }
	stage('Set Terraform path') {
	    ...
	    options {
                azureKeyVault(
                    credentialID: '	jenkins-sp-sql4', 
                    keyVaultURL: 'https://sqliptfstatekv-test-01.vault.azure.net/', 
                    secrets: [
		...
		terraform init -no-color -backend-config="storage_account_name=sqliptfstatestgtest" \
        -backend-config="container_name=sqliptfstate" \
